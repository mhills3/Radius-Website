...
// ("radiusdiscgolf.com/foundation"). Same instant, server-side platform redirect + GA4 attribution as
// /download, so installs the video drives get a clean, separable number instead of blurring into
// organic. Mobile → the right store with attribution tokens; desktop → a two-button landing.
// UA-dependent, so it must never be cached.
...
// GA4 Measurement Protocol — one `foundation_visit` event per hit so the video's traffic is countable.
  ...
  try {
    await fetch(`https://www.google-analytics.com/mp/collect?measurement_id=${GA_MEASUREMENT_ID}&api_secret=${secret}`, {
      ...
      body: JSON.stringify({
        ...
        events: [{ name: "foundation_visit", params: { source: SOURCE, platform, city: geo.city, region: geo.region, country: geo.country, engagement_time_msec: 1 } }],
      }),
    });
  } catch {
