...
type GTab = "overview" | "bag" | "improve";

const TABS: { key: GTab; label: string; icon: React.ReactNode }[] = [
  { key: "overview", label: "Overview", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4"><path d="M4 20V10M10 20V4M16 20v-7M22 20H2" /></svg> },
...
function MyGameInner() {
  ...
  const tab: GTab = tabParam === "bag" ? "bag" : tabParam === "improve" ? "improve" : "overview";
  const go = (t: GTab) => router.replace(t === "overview" ? "/bag" : `/bag?tab=${t}`, { scroll: false });
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-deep)] text-[var(--cream)]">
      ...
      <div className="mx-auto max-w-6xl px-6 py-6">
        {tab === "overview" && <MyGameOverview uid={user.uid} />}
        {tab === "bag" && <MyGameBag uid={user.uid} />}
        {tab === "improve" && <MyGameImprove uid={user.uid} />}
      </div>
    </div>
  );
}
