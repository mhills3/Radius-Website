...
export default function UrbanLanding({ appStore, googlePlay, qrSvg }: { appStore: string; googlePlay: string; qrSvg: string }) {
  ...
  useEffect(() => {
    ...
    if (/iPhone|iPad|iPod/i.test(ua)) {
      window.gtag?.("event", "urban_visit", { source: "urban", platform: "ios" });
      setRedirecting(true);
      window.location.replace(appStore);
      return;
    }
    if (/Android/i.test(ua)) {
      window.gtag?.("event", "urban_visit", { source: "urban", platform: "android" });
      setRedirecting(true);
      window.location.replace(googlePlay);
      return;
    }
    // desktop — count it under the same event so /urban has one clean number
    window.gtag?.("event", "urban_visit", { source: "urban", platform: "desktop" });
  }, [appStore, googlePlay]);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[var(--bg-deep)] px-6 py-16 text-[var(--cream)]">
      <div className="w-full max-w-md text-center">
        ...
        {/* Scan-to-install: a phone camera hits /urban and auto-redirects to the right store. This is
