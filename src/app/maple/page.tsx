...
import MapleLanding from "./MapleLanding";

// Dedicated CTA link for the Maple Hill creator video — memorable enough to say out loud
// ("radiusdiscgolf.com/maple"). Same instant, server-side platform redirect + GA4 attribution as
// /download, /foundation and /urban, so installs the video drives get a clean, separable number
// instead of blurring into organic. Mobile → the right store with attribution tokens; desktop → a
// two-button landing. UA-dependent, so it must never be cached.
...
const SOURCE = "maple";
const SELF_URL = "https://radiusdiscgolf.com/maple"; // what the desktop QR encodes → phone re-hits this and auto-redirects

type Geo = { city: string; region: string; country: string };

// GA4 Measurement Protocol — one `maple_visit` event per hit so the video's traffic is countable.
  ...
  try {
    await fetch(`https://www.google-analytics.com/mp/collect?measurement_id=${GA_MEASUREMENT_ID}&api_secret=${secret}`, {
      ...
      body: JSON.stringify({
        ...
        events: [{ name: "maple_visit", params: { source: SOURCE, platform, city: geo.city, region: geo.region, country: geo.country, engagement_time_msec: 1 } }],
      }),
    });
  } catch {
...
export default async function MaplePage() {
  ...
  // plus the store buttons. The client also fires the desktop maple_visit event.
  const qrSvg = await QRCode.toString(SELF_URL, { type: "svg", margin: 1, errorCorrectionLevel: "M", color: { dark: "#16221b", light: "#ffffff" } });
  return <MapleLanding appStore={APP_STORE} googlePlay={GOOGLE_PLAY} qrSvg={qrSvg} />;
}
