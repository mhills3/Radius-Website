...
/** Submit the claim through the server (rewardFulfillments becomes
 *  server-write-only; the callable re-derives identity + entitlement).
 *  DEPLOY ORDERING: this web code ships before the callable does — until
 *  the functions deploy lands, fall back to the legacy direct write,
 *  which the CURRENT production rules still allow. Once the new rules are
 *  live the callable exists and the fallback path is never taken (and
 *  would 403 if it were). Delete the fallback after deploy night. */
export async function submitFulfillment(claim: Claimable, input: FulfillmentInput): Promise<void> {
  try {
    ...
    if (!/not-found|unimplemented|internal/.test(code)) throw e;
