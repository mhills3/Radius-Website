  ...
  return (
    <Link href={`/community/thread/${thread.id}`} className="block w-full rounded-2xl border border-white/[0.07] bg-white/[0.03] p-4 text-left transition-all hover:-translate-y-0.5 hover:border-white/[0.12]">
      ...
      <div className="mt-2.5 flex items-center gap-4 text-xs text-[var(--sage-dim)]">
        {thread.score > 0 && (
          <span className="inline-flex items-center gap-1"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="h-3.5 w-3.5"><path d="M5 15l7-7 7 7" /></svg>{thread.score}</span>
        )}
