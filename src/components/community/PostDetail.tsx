        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="border-b border-white/[0.07] p-5">
            ...
            {post.linkedCourseName && (
              <div className="mt-3 overflow-hidden rounded-xl border border-white/[0.07] bg-white/[0.03]">
                <div className="flex items-center gap-3 p-3">
                  <span className="relative grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-lg bg-[radial-gradient(circle_at_30%_25%,rgba(246,193,101,0.3),var(--bg-deep))] font-[family-name:var(--font-heading)] font-bold text-[var(--cream)]/70">
                    {post.linkedCourseName.charAt(0).toUpperCase()}
                    {post.linkedCourseCover && (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={post.linkedCourseCover} alt="" className="absolute inset-0 h-full w-full object-cover" onError={(e) => e.currentTarget.remove()} />
                    )}
                  </span>
                  <div className="min-w-0 flex-1"><div className="truncate font-bold">{post.linkedCourseName}</div><div className="text-xs text-[var(--sage-dim)]">Round{post.holesPlayed ? ` · ${post.holesPlayed} holes` : ""}{post.linkedBirdies ? ` · ${post.linkedBirdies} birdie${post.linkedBirdies === 1 ? "" : "s"}` : ""}</div></div>
                ...
                {hasScorecard && (
                  <button onClick={() => setScorecard(true)} className="flex w-full items-center justify-center gap-1.5 border-t border-white/[0.07] py-2 text-[12px] font-bold uppercase tracking-wide text-[var(--gold)] transition-colors hover:bg-[var(--gold)]/[0.08]">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-3.5 w-3.5"><rect x="4" y="3" width="16" height="18" rx="2" /><path d="M8 8h8M8 12h8M8 16h5" /></svg>
                    View scorecard
                  </button>
                )}
              </div>
