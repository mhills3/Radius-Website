...
// Branded gradient palette — used as the fallback behind a course's cover photo (and if it fails).
      ...
      <div className="mx-auto max-w-7xl px-6 py-20 md:py-24">
        ...
        <div className="mt-10 text-center">
          <Link href="/courses" className="inline-flex items-center gap-2 rounded-full bg-[#16221b] px-7 py-3.5 text-sm font-bold text-[var(--cream)] transition-all hover:-translate-y-0.5 hover:bg-[#22332a]">
            Browse all {loading && !totalCount ? "" : (totalCount || courses.length).toLocaleString()} courses →
          </Link>
        </div>
      </div>
    </section>
...
function CourseTile({ c, featured = false, className = "" }: { c: Course; featured?: boolean; className?: string }) {
  ...
  return (
    <Link href={`/courses/${slugify(c.name, c.id)}`} className={`group relative overflow-hidden rounded-3xl shadow-sm ring-1 ring-inset ring-white/10 transition-all hover:-translate-y-0.5 hover:shadow-[0_24px_60px_-22px_rgba(0,0,0,0.45)] ${className}`} style={{ background: `linear-gradient(140deg, ${color}, #0d140f 80%)` }}>
      ...
      {/* big faint hole count for depth (fallback layer — covered by the photo when present) */}
      ...
      {/* cover photo — removes itself on error so the branded gradient shows through */}
      {c.coverPhotoUrl && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={c.coverPhotoUrl} alt="" loading="lazy" decoding="async" className="absolute inset-0 h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" onError={(e) => e.currentTarget.remove()} />
      )}
      {/* legibility gradient over the photo */}
      <div className="absolute inset-0 bg-[linear-gradient(to_top,rgba(13,20,15,0.9),rgba(13,20,15,0.1)_55%,transparent)]" />
