  ...
  const inner = (
      <div className="overflow-hidden rounded-2xl border border-white/[0.12] bg-[var(--bg-mid)] shadow-[0_24px_60px_-24px_rgba(0,0,0,0.95)] ring-1 ring-black/20 transition-all duration-300 group-hover/card:-translate-y-1.5 group-hover/card:border-[var(--gold)]/40 group-hover/card:shadow-[0_32px_72px_-24px_rgba(246,193,101,0.32)]">
        <div className="relative aspect-video w-full overflow-hidden">
          ...
          {/* the one featured partner gets a badge */}
          {v.partner ? (
            <span className="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full border border-white/15 bg-black/55 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider backdrop-blur-md" style={{ color: v.accent }}>★ Featured partner</span>
          ) : null}
        ...
        <div className="px-4 pb-3.5 pt-3">
          ...
          <div className="flex items-center gap-2">
            <span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: v.accent || (v.own ? "var(--gold)" : "var(--sage)") }} />
            <span className="min-w-0 flex-1 truncate text-[11.5px] font-bold" style={{ color: v.accent || (v.own ? "var(--gold)" : "var(--sage)") }}>{v.channel}</span>
