...
const MONO = { fontFamily: "'Sora', sans-serif" } as const; // all numerals
      ...
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="block">
        <g transform={`translate(${size / 2} ${size / 2})`}>
          ...
          <text textAnchor="middle" dominantBaseline="central" y="0">
            <tspan style={{ fontSize: size * 0.235, fontWeight: 800, fontFamily: "'Sora', sans-serif", fill: "var(--cream)" }}>{value}</tspan>
            {unit && <tspan dx="1" style={{ fontSize: size * 0.13, fontWeight: 700, fontFamily: "'Sora', sans-serif", fill: "var(--sage-dim)" }}>{unit}</tspan>}
          </text>
        </g>
      </svg>
