...
export default function CourseCommunity({ courseId, courseName }: { courseId: string; courseName: string }) {
  ...
  return (
    <section className="mt-10">
      <h2 className="mb-1 font-[family-name:var(--font-heading)] text-xl font-bold tracking-tight text-[var(--c-ink)]">💬 {courseName} community</h2>
      <p className="mb-4 text-sm text-[var(--c-muted)]">Join the group, ask about conditions, share layouts & tips — your home course auto-joins you.</p>

      <CourseGroupBar courseId={courseId} courseName={courseName} />
      <CourseDiscussionForm courseId={courseId} courseName={courseName} />

      {posts === null ? (
        <div className="mt-5 space-y-3">{[0, 1].map((i) => <div key={i} className="h-16 animate-pulse rounded-2xl bg-[var(--c-raise)]" />)}</div>
      ) : posts.length > 0 ? (
        ...
        <p className="mt-5 text-sm text-[var(--c-muted)]">No posts yet — start the conversation about {courseName}.</p>
      )}

      {mentions.length > 0 && (
        <div className="mt-8">
          <h3 className="mb-3 text-xs font-bold uppercase tracking-wide text-[var(--c-muted)]">🗣️ Mentioned in the community feed</h3>
          <div className="space-y-3">
            {mentions.map((m) => (
              <Link key={m.id} href={`/community/post/${m.id}`} className="flex gap-3 rounded-2xl border border-[var(--c-line)] bg-[var(--c-card)] p-3.5 shadow-sm transition-colors hover:border-[var(--gold)]">
                <span className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-full bg-[var(--bg-mid)] text-xs font-bold text-[var(--cream)]">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-bold text-[var(--c-ink)]">{m.authorName} <span className="font-normal text-[var(--c-muted)]">· {timeAgo(m.createdAt)}</span></div>
                  <p className="mt-0.5 line-clamp-2 text-sm leading-relaxed text-[var(--c-body)]">{m.text}</p>
                </div>
              </Link>
            ))}
          </div>
